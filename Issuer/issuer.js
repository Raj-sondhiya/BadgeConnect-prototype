...
// Dynamically load navbar on issuerHome.html
if (window.location.pathname.endsWith("issuerHome.html")) {
...
// Dynamically load navbar on addEarner.html
if (window.location.pathname.endsWith("addEarner.html")) {
    fetch("issuerNavbar.html")
        .then(response => response.text())
        .then(html => {
            const navbarContainer = document.createElement("div");
...
// Dynamically load navbar on addEarner.html
if (window.location.pathname.endsWith("earnerList.html")) {
    fetch("issuerNavbar.html")
        .then(response => response.text())
        .then(html => {
            const navbarContainer = document.createElement("div");
        ...
        earners.forEach((earner, index) => {
            ...
            row.innerHTML = `
                ...
                <td class="text-center">
                    <i class="fa-solid fa-eye text-primary me-2" style="cursor:pointer;" title="View" onclick="viewEarner(${index})"></i>
                    <i class="fa-solid fa-trash text-danger" style="cursor:pointer;" title="Delete" onclick="deleteEarner(${index})"></i>
                </td>
            `;
